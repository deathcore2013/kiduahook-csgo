...
bool __fastcall HOOKED_CreateMove( void* thisptr, int edx, float frametime, CUserCmd* pCmd )
{
	...
	if (GetAsyncKeyState(g_Var.Misc.FakeDuck) && g_Var.Misc.FakeDuck > 0 && (UserCmd::iOriginalFlags & FL_ONGROUND))
	{
		...
		if (g_pClientState->m_choked_commands >= (iDrop / 2))
			pCmd->buttons |= IN_DUCK;
		else
			pCmd->buttons &= ~IN_DUCK;

		if (iDrop > g_pClientState->m_choked_commands)
			bSendPackets = false;
	}
	else
	...
	if( !g_pClientState->m_choked_commands )
	{
	...
	if( g_pClientState->m_choked_commands >= 14 )
	{
		bSendPackets = true;
