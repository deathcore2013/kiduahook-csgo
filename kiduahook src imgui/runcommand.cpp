...
void CRunCommand::RunPrediction( CBaseEntity *pLocal, CUserCmd *pUserCmd )
{
	...
	if (g_pClientState->m_delta_tick > 0)
		g_pPred->Update(g_pClientState->m_delta_tick, g_pClientState->m_delta_tick > 0, g_pClientState->m_last_command_ack, g_pClientState->m_last_outgoing_command + g_pClientState->m_choked_commands);
