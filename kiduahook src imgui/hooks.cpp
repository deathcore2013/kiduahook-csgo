...
//	oInPrediction = ( InPrediction_t ) g_pRunCommandHOOKED->dwHookMethod( ( DWORD ) HOOKED_InPrediction, 14 );
	...
	//shonax
	static auto p_device = **reinterpret_cast<IDirect3DDevice9***>(g_Pattern.PatternSearchIDA("shaderapidx9.dll", "A1 ? ? ? ? 50 8B 08 FF 51 0C") + 0x1);
	shonax_d3dx9_hooked = new CVMTHookManager((PDWORD*)p_device);
	oEndSceneResetFn = (EndSceneResetFn)shonax_d3dx9_hooked->dwHookMethod(reinterpret_cast<uintptr_t>(hooked_reset), 16);
	oPresentFn = (PresentFn)shonax_d3dx9_hooked->dwHookMethod(reinterpret_cast<uintptr_t>(hooked_present), 17);
	//shonax
